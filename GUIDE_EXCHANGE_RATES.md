...
- Сервер каждые 5 минут ходит на сторонний сайт за свежими курсами
...
┌──────────────────┐          ┌──────────────────┐          ┌──────────────────────┐
│                  │          │                  │          │ Таблица balances:    │
│  Даёт актуальные │  <────── │  Каждые 5 мин    │  ──────> │   id | currency      │
...
Это и есть те курсы, которые наш сервер будет забирать каждые 5 минут.
...
- `"time"` — для таймера (каждые 5 минут)
...
### 3.6. Добавляем таймер (каждые 5 минут)
...
- Каждые 5 минут обновлять курсы

```go
// startRateUpdater запускает фоновое обновление курсов каждые 5 минут
func startRateUpdater() {
    // Сразу обновляем курсы при старте сервера (не ждём 5 минут)
    fetchAndSaveRates()

    // Создаём "тикер" — он будет "тикать" каждые 5 минут
    ticker := time.NewTicker(5 * time.Minute)

    // Запускаем горутину (параллельную задачу)
    go func() {
        for range ticker.C {
            // Каждые 5 минут вызываем функцию обновления курсов
            fetchAndSaveRates()
        }
    }()
}
...
onMounted(() => {
  ...
  // Обновляем курсы каждые 5 минут (чтобы на экране были свежие)
...
5. **Курсы обновляются?**
   Подожди 5 минут, проверь в консоли Go-сервера — должно снова появиться
   "Курсы валют обновлены успешно!".
